...
for m in metrics:
    ...
    plt.ylabel('% of viewpoints better than 2D', fontsize = 26)
    plt.xlabel('Layout Technique', fontsize=26)
    plt.xticks(fontsize = 22)
    plt.yticks(fontsize = 22)
    plt.savefig('evaluations/indiv_metric_' + str(metrics_mapping[m]) + '.png')
    plt.tight_layout()
    plt.clf()
    plt.close('all')
...
fig, axes = plt.subplots(1, 5, figsize = (1000 / 80, 1000 / 40), dpi = 96)
for i, col in enumerate(metrics[:5]):
...
plt.savefig('evaluations/all_values_agg_rel_boxplot_norm_p1.png')
plt.clf()
plt.close('all')
