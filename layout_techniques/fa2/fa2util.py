...
def linRepulsion(n1, n2, dim, coefficient=0):
    xDist = n1.x - n2.x
    yDist = n1.y - n2.y
...
def strongGravity(n, g, dim, coefficient=0):
    xDist = n.x
    yDist = n.y
...
# not return anything.
def linAttraction(n1, n2, e, distributedAttraction, dim, coefficient=0):
...
def apply_repulsion(nodes, dim, coefficient):
    i = 0
    for n1 in nodes:
        j = i
        for n2 in nodes:
            if j == 0:
                break
            linRepulsion(n1, n2, dim, coefficient)
            j -= 1
        i += 1


def apply_gravity(nodes, gravity, scalingRatio, dim, useStrongGravity=False):
    if not useStrongGravity:
        for n in nodes:
            linGravity(n, gravity, dim)
    else:
        for n in nodes:
            strongGravity(n, gravity, dim, scalingRatio)


def apply_attraction(nodes, edges, distributedAttraction, coefficient, edgeWeightInfluence, dim):

    # Optimization, since usually edgeWeightInfluence is 0 or 1, and pow is slow
    if edgeWeightInfluence == 0:
        for edge in edges:
            linAttraction(nodes[edge.node1], nodes[edge.node2], 1, distributedAttraction, dim, coefficient)
    elif edgeWeightInfluence == 1:
        for edge in edges:
            linAttraction(nodes[edge.node1], nodes[edge.node2], edge.weight, distributedAttraction, dim, coefficient)
    else:
        for edge in edges:
            linAttraction(nodes[edge.node1], nodes[edge.node2], pow(edge.weight, edgeWeightInfluence),
                          distributedAttraction, dim, coefficient)


# For Barnes Hut Optimization
# Barnes Hut Optimization is not used for 3d so it does not have a dimension parameter
class Region:
...
def adjustSpeedAndApplyForces(nodes, speed, speedEfficiency, jitterTolerance, dim):
