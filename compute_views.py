...
def compute_views(proj_file_name, viewpoints_list, labels_list = None, display = False, manual_rotate = False):

    # get a colormapping
    cmap = plt.get_cmap('tab10')

    df = pd.read_csv(proj_file_name, sep=';', header=0)
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    if labels_list is not None:
        ax.scatter(df['x'], df['y'], df['z'], s=10, c=[cmap(cl) for cl in labels_list])
    else:
        ax.scatter(df['x'], df['y'], df['z'], s=10)

    viewpoints_spherical = utils.rectangular_to_spherical(viewpoints_list)
    ...
    viewsfile = '{0}/{1}.pkl'.format(constants.output_dir, os.path.basename(proj_file_name).replace('3d.csv', 'views'))
    ...
    if manual_rotate:
        ...
        for index, view_sphere in enumerate(viewpoints_spherical):
            ...
            if display:
                ...
                eye = viewpoints_list[index] * 100
                update_view(view_proj, view, cmap, df, eye, fig2)
                plt.pause(1)

        views_df = pd.DataFrame.from_records([(np.array(p), views[index]) for index, p in enumerate(viewpoints_list)])
...
def update_view(ax, view, cmap, df, eye, fig):
    ...
    # ccalculate the 2d view and display it in the plot
    ax.clear()
    sorted_indices = np.argsort(-np.linalg.norm(df.to_numpy() - eye, axis = 1))

    if labels is not None:
        ax.scatter(view[sorted_indices, 0], view[sorted_indices, 1], s = 10, c = [cmap(cl) for cl in labels[sorted_indices]])
    else:
